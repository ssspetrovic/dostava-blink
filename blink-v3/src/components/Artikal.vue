<template>
  <section>
    <div class="container vh-100">
      ...
      <router-link
        :to="{ path: '/edit-article/' + artikal.id }"
        class="btn btn-primary btn-lg"
        >MENADZER: Izmeni?</router-link
      >
      <router-link to="/create-order" class="btn btn-primary btn-lg"
        >KUPAC: Naruci?</router-link
      >
      <a v-on:click="logout" href="#" class="btn btn-primary btn-lg"
        >MENADZER: Obrisi Artikal</a
      >
    </div>
  </section>
</template>
...
export default {
  ...
  methods: {
    ...
    logout() {
      axios
        .delete(
          `http://localhost:8080/api/artikal/obrisi/` +
